...
class TranscriptEmitter:
    ...
    def emit_transcript(self, merged: "MergedTranscript", current_duration: float, finalize: bool) -> None:
        if not merged.turns:
            return
        ...
        with self._lock:
            ...
            for turn in merged.turns:
                if turn.speaker not in self._seen_speakers:
                    self._seen_speakers.add(turn.speaker)
                    new_speakers = True
...
class LiveProcessor:
    ...
    def _maybe_process(self, finalize: bool) -> bool:
        ...
        self._emitter.emit_transcript(merged, duration, finalize=finalize)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=["system", "mic"],
        default="system",
        help="Which stream to transcribe (default: system)",
    )
    parser.add_argument(
        "--diar-backend",
...
def main() -> int:
    ...
    live_processor = LiveProcessor(
        stream_name=args.transcribe_stream,
        state=state,
        emitter=emitter,
        ...
        if msg_type == MSG_MEETING_START:
            ...
            with state.lock:
                if stream_id == STREAM_SYSTEM and state.system_writer:
                    ...
                    if not args.no_live and args.transcribe_stream == "system":
                        duration = state.system_writer.last_sample_index / float(state.sample_rate)
                        live_processor.notify_duration(duration)
                    ...
                    if not args.no_live and args.transcribe_stream == "mic":
    ...
    selected_writer = system_writer if args.transcribe_stream == "system" else mic_writer
    if not selected_writer or selected_writer.bytes_written == 0:
        emit_jsonl({
            "type": "error",
            "message": f"no_audio_for_stream_{args.transcribe_stream}",
        }, stdout_lock)
        return 1

    if args.no_live:
        snapshot = snapshot_stream(selected_writer, state.sample_rate, state.channels)
        ...
        duration = selected_writer.last_sample_index / float(state.sample_rate)
        emitter.emit_transcript(merged, duration, finalize=True)
